...
admin (HTTP :3001, internal/upload)
   → uploads EPUB to MinIO, posts to "split"
...
### Storage layout (MinIO buckets, see `internal/primitive/buckets.go`)
...
`internal/book` is the Book *domain* package: the `Book`/`BookDTO`/`Stage` model (`model.go`), the `Repository`/`BucketRepo`/`QueueRepo` ports and their MySQL/MinIO/RabbitMQ implementations (`repository.go`, `bucket_repository.go`, `queue_repository.go`), plus the `OpenAiClient` shared by the two AI-driven stages (`openai_client.go`). It has no `Service` and no HTTP handlers of its own — every pipeline stage and read path is a separate *use-case* package built on top of it:
